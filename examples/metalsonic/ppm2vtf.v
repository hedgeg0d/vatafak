...
fn main() {
	args := os.args
	if args.len < 2 {
		eprintln('specify ppm image')
		exit(0)
	}
	lines := os.read_lines(args[1]) or { panic(err) }
	if lines[0] != 'P3' || lines[1] != '100 100' {
		eprintln('P3 ppm image 100x100 required.')
		exit(0)
	}
	os.mkdir('${args[1].split('.')[0]}') or {
		eprintln('failed to create folder ${args[1].split('.')[0]}')
		exit(0)
	}
	os.chdir('${args[1].split('.')[0]}') or {
		panic(err)
		exit(0)
	}
	if os.exists('${args[1].split('.')[0]}.vtf') {
		os.rm(args[1].split('.')[0] + '.vtf') or { panic(err) }
	}
	...
	w, h := strconv.atoi(lines[1].split(' ')[0]) or { 0 }, strconv.atoi(lines[1].split(' ')[1]) or {
		0
	}
